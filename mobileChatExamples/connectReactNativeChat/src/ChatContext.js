...
function request(url, options, timeout = 5000) {
  return Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(() => reject({ networkError: 'Request timeout' }), timeout)
    )
  ]).then(response => {
    if (!response.ok) throw response;
    return response;
  }).catch(error => {
    throw error.networkError ? error : { networkError: error.message || 'Network error' };
  });
}
...
export const ChatProvider = ({ children, customerName = "Joe Shmoe" }) => {
  ...
  const fetchChatDetails = async () => {
    ...
    const chatDetails = response.json.data;
  ...
  }, [initializeChat, contactStatus, isLoading]);
