...
function handleRequest(request) {
  ...
  request.sessionAttributes = request.sessionState.sessionAttributes;
  let current_intent = request.currentIntent.name;

  /* HANDLE INTENT 'InteractiveMessageIntent' */
  if (current_intent === 'InteractiveMessageIntent' && !recent_intent.slots.action) {
    return handleElicitAction(request);
  } else if (current_intent === 'InteractiveMessageIntent' && !recent_intent.slots.interactiveOption) {
    return handleActionResponse(input, request);
  } else if (current_intent === 'InteractiveMessageIntent' && Object.values(TEST_INTERACTIVE_OPTIONS).includes(input)) {
